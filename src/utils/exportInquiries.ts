...
export const exportInquiriesToExcel = (
  ...
  const headers = [
    ...
    'AI评分 (AI Score)',
    '机器人对话轮数 (Bot Turns)',
    '售前机器人接待摘要 (Bot Summary)',
    '客户进线消息 (Customer Message)',
    '售前机器人回复方案 (Bot Reply / FOB Proposal)'
  ];
  ...
  const rows = items.map((item) => {
    ...
    const botReply = item.aiReplyDraft || item.aiAnalysis?.suggestedReply || '';
    const summary = item.aiAnalysis?.summary || '';
    ...
    return [
      ...
      escapeCSV(item.intentLevel),
      escapeCSV(item.aiScore),
      escapeCSV(chatTurns),
      escapeCSV(summary),
      escapeCSV(item.rawContent || item.content),
      escapeCSV(botReply)
    ].join(',');
  });
  ...
  const filename = customFilename || `HomeCraft_WhatsApp售前询盘客户表_${dateSuffix}.csv`;
